...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.t === 'join') {
      const newRoomId = String(msg.room || 'main').slice(0, 40);
      if (ws.roomId && ws.roomId !== newRoomId) {
        // leave the previous room cleanly before joining the new one
        const oldRoom = rooms.get(ws.roomId);
        if (oldRoom) {
          oldRoom.clients.delete(ws);
          for (const key of ws.ownedKeys) {
            delete oldRoom.state[key];
            broadcast(oldRoom, { t: 'del', key });
          }
